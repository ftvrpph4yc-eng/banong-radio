...
| Sanitizer | `RawTextItem` iterable | `SanitizedTextItem` list | Filter empty text, deduplicate by `item_id`, redact sensitive fields, and stop before signal or plan generation |
...
- The preserved real-source registry keys are `wechat_group`, `weather_api`, `government_website`, `voice_transcript`, and `community_source`. They are extension interfaces, not live data connections.
