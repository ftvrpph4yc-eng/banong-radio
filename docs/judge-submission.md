...
| Technical quality | `SourceAdapter -> Sanitizer -> SignalExtractor -> ContextBuilder -> TaskPlanner -> RadioPlanner/TextOutputGenerators`, then TTS/music/mix/playback/status for radio |
| Completion | Runnable CLI, generated demo feed manifest, text output pack, local playback, dashboard, fallback, tests, and documented acceptance commands |
...
3. The code turns a synthetic village feed into sanitized items, signals, context, task briefs, a runtime `BroadcastPlan`, and a separate text output pack.
